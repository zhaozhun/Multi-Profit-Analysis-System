...
import { Card, Table, Button, Space, Tabs, Tag, Modal, Form, Input, Select, message, Spin } from 'antd';
...
const categories = [
  ...
  { key: 'SCALE', label: '规模类', color: '#52c41a' },
  { key: 'REV', label: '收入类', color: '#1890ff' },
  { key: 'COST', label: '成本类', color: '#fa8c16' },
  { key: 'PROFIT', label: '利润类', color: '#f5222d' },
  { key: 'EFF', label: '效率类', color: '#722ed1' },
];
...
const Indicator: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const result: any = await api.get('/indicator', { params: { category: activeCategory } });
      setData(result?.list || []);
    } catch (error) {
  ...
  const getCategoryColor = (cat: string) => categories.find(c => c.key === cat)?.color || '#1890ff';
  ...
  const columns = [
    ...
    { title: '分类', dataIndex: 'category', width: 100, render: (v: string) => (
      <Tag color={getCategoryColor(v)}>{getCategoryLabel(v)}</Tag>
    )},
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <h2 style={{ margin: 0 }}>📊 指标库管理</h2>
        <p style={{ color: '#999', margin: '4px 0 0' }}>管理分析指标，配置预计算周期</p>
      </div>
