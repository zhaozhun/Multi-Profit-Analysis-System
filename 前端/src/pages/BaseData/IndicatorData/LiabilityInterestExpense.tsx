...
const LiabilityInterestExpense: React.FC = () => {
  ...
  const getBarChartOption = () => {
    ...
    return {
      ...
      legend: { data: ['余额(万)', '利息支出(万)'] },
      grid: { left: '3%', right: '4%', bottom: '3%', containLabel: true },
      xAxis: { type: 'category', data: names, axisLabel: { rotate: 30 } },
      yAxis: [
        { type: 'value', name: '余额(万)', position: 'left' },
        { type: 'value', name: '利息支出(万)', position: 'right' },
      ],
      series: [
        { name: '余额(万)', type: 'bar', data: balances, itemStyle: { color: '#1890ff' } },
        { name: '利息支出(万)', type: 'bar', yAxisIndex: 1, data: interests, itemStyle: { color: '#ff4d4f' } },
      ],
    };
  };
  ...
  const getPieChartOption = () => {
    const data = dimensionData.map(item => ({ name: item.dim_name, value: item.total_interest }));
    return {
      tooltip: { trigger: 'item', formatter: '{a} <br/>{b}: {c} ({d}%)' },
      legend: { orient: 'vertical', left: 'left' },
      series: [{ name: '利息支出', type: 'pie', radius: '50%', data: data }],
    };
  };
  ...
  const summaryColumns = [
    ...
    { title: '当日利息支出', dataIndex: 'dailyInterest', key: 'dailyInterest', width: 130, render: (val: number) => (val / 10000).toFixed(4) + ' 万' },
    { title: '当月利息支出', dataIndex: 'monthlyInterest', key: 'monthlyInterest', width: 130, render: (val: number) => (val / 10000).toFixed(2) + ' 万' },
    { title: '当年利息支出', dataIndex: 'yearlyInterest', key: 'yearlyInterest', width: 130, render: (val: number) => (val / 10000).toFixed(2) + ' 万' },
  ...
  const detailColumns = [
    ...
    { title: '存款利率', dataIndex: 'depositRate', key: 'depositRate', width: 100, render: (val: number) => val ? (val * 100).toFixed(2) + '%' : '-' },
    { title: '当日利息', dataIndex: 'depositDailyInterest', key: 'depositDailyInterest', width: 100, render: (val: number) => val ? (val / 10000).toFixed(4) + '万' : '-' },
    { title: '当月利息', dataIndex: 'depositMonthlyInterest', key: 'depositMonthlyInterest', width: 100, render: (val: number) => val ? (val / 10000).toFixed(4) + '万' : '-' },
    { title: '累计利息', dataIndex: 'depositCumulativeInterest', key: 'depositCumulativeInterest', width: 100, render: (val: number) => val ? (val / 10000).toFixed(4) + '万' : '-' },
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <h2 style={{ margin: 0 }}>对客利息支出</h2>
        <p style={{ color: '#999', margin: '4px 0 0' }}>负债条线对客利息支出指标及存款余额信息</p>
      </div>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={14}>
          <Card title={`${DIMENSIONS.find(d => d.value === dimension)?.label}维度余额与利息支出分布`}>
...
export default LiabilityInterestExpense;
