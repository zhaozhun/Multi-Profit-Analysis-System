...
const AssetFtpCost: React.FC = () => {
  ...
  const getBarChartOption = () => {
    ...
    const ftpCosts = dimensionData.map(item => (item.total_ftp_cost / 10000).toFixed(2));

    return {
      tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
      legend: { data: ['余额(万)', 'FTP成本(万)'] },
      grid: { left: '3%', right: '4%', bottom: '3%', containLabel: true },
      xAxis: { type: 'category', data: names, axisLabel: { rotate: 30 } },
      yAxis: [
        { type: 'value', name: '余额(万)', position: 'left' },
        { type: 'value', name: 'FTP成本(万)', position: 'right' },
      ],
      series: [
        { name: '余额(万)', type: 'bar', data: balances, itemStyle: { color: '#1890ff' } },
        { name: 'FTP成本(万)', type: 'bar', yAxisIndex: 1, data: ftpCosts, itemStyle: { color: '#ff4d4f' } },
      ],
    };
  };
  ...
  const getPieChartOption = () => {
    const data = dimensionData.map(item => ({ name: item.dim_name, value: item.total_ftp_cost }));
    return {
      tooltip: { trigger: 'item', formatter: '{a} <br/>{b}: {c} ({d}%)' },
      legend: { orient: 'vertical', left: 'left' },
      series: [{ name: 'FTP成本', type: 'pie', radius: '50%', data: data }],
    };
  };

  const summaryData = [{
    key: 'summary',
    ftpCost: summary.total_ftp_cost || 0,
    avgFtpRate: summary.avg_rate ? summary.avg_rate * 0.35 : 0,
    balance: summary.total_balance || 0,
  }];

  const summaryColumns = [
    { title: 'FTP成本合计', dataIndex: 'ftpCost', key: 'ftpCost', width: 150, render: (val: number) => (val / 10000).toFixed(2) + ' 万' },
    { title: '平均FTP利率', dataIndex: 'avgFtpRate', key: 'avgFtpRate', width: 120, render: (val: number) => (val * 100).toFixed(2) + '%' },
  ...
  const dimensionColumns = [
    ...
    { title: 'FTP成本合计', dataIndex: 'total_ftp_cost', key: 'total_ftp_cost', width: 130, render: (val: number) => val ? (val / 10000).toFixed(2) + '万' : '-' },
  ];
  ...
  const detailColumns = [
    ...
    { title: 'FTP成本', dataIndex: 'ftpCost', key: 'ftpCost', width: 100, render: (val: number) => val ? (val / 10000).toFixed(2) + '万' : '-' },
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <h2 style={{ margin: 0 }}>FTP成本</h2>
        <p style={{ color: '#999', margin: '4px 0 0' }}>资产条线FTP资金成本</p>
      </div>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={14}>
          <Card title={`${DIMENSIONS.find(d => d.value === dimension)?.label}维度余额与FTP成本分布`}>
